...
class switchClass():
    ...
    def commit_config(self):
            ...
            self.addtemplate(basetemplate)

            # Write config for stack
            configout = open(tftproot + self.mac + '.txt', 'w')
            configout.write(self.config)
            configout.close()
...
with open(basename, 'r') as csvfile:
    ...
    # Write final config if last line is null
    if configwritten == 0:
        newswitch.commit_config()
